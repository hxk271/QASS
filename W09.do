...
*interaction terms practice 4
	...
	*visualization
	graph twoway (scatter hapinss1 age, jitter(5)) ///
				 (lfit hapinss1 age if married==1, lcolor(red))   ///
				 (lfit hapinss1 age if married==0, lcolor(blue)), legend(off) 

	*marginsplot
	margins, at(age=(30(1)80)) by(married)
	marginsplot
	marginsplot, recast(line) recastci(rline) ciopts(color(gray%40))

	*partial derivative (dydx)
	margins, dydx(married) at(age=(18(1)95))
	marginsplot, recast(line) yline(0) recastci(rline) ciopts(color(gs10)) 
